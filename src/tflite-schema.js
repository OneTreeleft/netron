...
$root.tflite.Tensor = class Tensor {
    ...
    static decode(reader, position) {
        ...
        $.quantization = reader.table_(position, 12, $root.tflite.QuantizationParameters.decode);
        $.is_variable = reader.bool_(position, 14, false);
        $.sparsity = reader.table_(position, 16, $root.tflite.SparsityParameters.decode);
    ...
    static decodeText(reader, json) {
        ...
        $.quantization = reader.value_(json.quantization, $root.tflite.QuantizationParameters.decodeText);
        $.is_variable = reader.value(json.is_variable, false);
        $.sparsity = reader.value_(json.sparsity, $root.tflite.SparsityParameters.decodeText);
...
$root.tflite.ImageProperties = class ImageProperties {
    ...
    static decode(reader, position) {
        ...
        $.default_size = reader.table_(position, 6, $root.tflite.ImageSize.decode);
        return $;
    }
};
...
$root.tflite.Content = class Content {
    ...
    static decode(reader, position) {
        ...
        $.range = reader.table_(position, 8, $root.tflite.ValueRange.decode);
        return $;
    }
};
...
$root.tflite.TensorMetadata = class TensorMetadata {
    ...
    static decode(reader, position) {
        ...
        $.content = reader.table_(position, 10, $root.tflite.Content.decode);
        $.process_units = reader.array_(position, 12, $root.tflite.ProcessUnit.decode);
        $.stats = reader.table_(position, 14, $root.tflite.Stats.decode);
